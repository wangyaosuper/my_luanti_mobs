...
local function embed_add_prefix(node, name, prefix)
    ...
    if node.on_close_popover then
        node.on_close_popover = embed_wrap_callback_func(node.on_close_popover, name, prefix)
    end
    ...
    if node.popover then
        embed_add_prefix(node.popover, name, prefix)
    end
end
